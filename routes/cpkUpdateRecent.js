			...
			if (err) {
					...
					if (err2) {
						...
						testUserProfile.findOne({user_id: req.body.userID }, function(err3, rslt3) {
							if (err3) {
								...
								var tmpFavs = rslt3.recently_watched.split(';');
								...
								if (rslt3.recently_watched.indexOf(rslt2._id) == -1) {
									rslt3.recently_watched = rslt3.recently_watched + rslt2._id + ';';
				...
				testUserProfile.findOne({user_id: req.body.userID }, function(err2, rslt2) {
					...
					if (err2) {
						...
						var tmpFavs = rslt2.recently_watched.split(';');
						if (tmpFavs.length > 20) {
						...
						if (rslt2.recently_watched.indexOf(rslt._id) == -1) {
							rslt2.recently_watched = rslt2.recently_watched + rslt._id + ';';						
